...
hl.bind(mainMod .. " + G", hl.dsp.exec_cmd("remotegamepad &"))
...
hl.bind("ALT + R", hl.dsp.exec_cmd("hyprctl reload && notify-send 'Hyprland' 'Config reloaded'"),            { locked = true, repeating = false })
for i = 1, 10 do
  local key = i % 10
...
hl.bind("SUPER + O", hl.dsp.exec_cmd("systemctl poweroff"), { locked = true, repeating = false })
hl.bind("SUPER + I", hl.dsp.exec_cmd("systemctl reboot"), { locked = true, repeating = false })
hl.bind("SUPER + SHIFT + K", hl.dsp.exec_cmd("pkill kdeconnect-app || kdeconnect-app"), { repeating = false })

hl.bind("SUPER + SHIFT + R", hl.dsp.exec_cmd("bash ~/.config/Scripts/screen_recorder.sh"),      { locked = true, repeating = false })
